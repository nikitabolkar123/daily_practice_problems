...
# ruleKey = "color", ruleValue = "silver" Output: 1 Explanation: There is only one item matching the given rule,
# which is ["computer","silver","lenovo"]. Example 2:
#
# Input: items = [["phone","blue","pixel"],["computer","silver","phone"],["phone","gold","iphone"]],
# ruleKey = "type", ruleValue = "phone" Output: 2 Explanation: There are only two items matching the given rule,
# which are ["phone","blue","pixel"] and ["phone","gold","iphone"]. Note that the item ["computer","silver",
...
def count_match(items, ruleKey, ruleValue):
    return sum([1 for item in items if (ruleKey == "type" and item[0] == ruleValue) or
